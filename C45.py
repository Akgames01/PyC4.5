...
# -> By running the C45_GUI.py to have a GUI to visualise results
...
def getInformationGain(data, column):
    ...
    entropy_above = []
    entropy_below = []
    target_entropy = getEntropy(data, 'type')  # get target entropy for the dataset
    # get entropy for sets above and below each of the thresholds
    for set in sets_below:
        entropy_below.append(getEntropy(set, 'type'))
        instances_below.append(len(set))
    for set in sets_above:
        entropy_above.append(getEntropy(set, 'type'))
...
def train(data):
    ...
    if len(best['left']) != 0 and len(best['right']) != 0:
        ...
        label = list(getUniqueClasses(data['type']).keys()); #get label for the leaf node
...
def main():
    ...
    for i in range(tests):
        ...
        types = test_data['type'] # get types column from test_data
        del test_data['type'] # deletes labels from test_data so it cannot be used in classification
